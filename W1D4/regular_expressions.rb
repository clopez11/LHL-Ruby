...
def grab_sin(string)
  sin_number = nil
  if has_sin?(string)
    result = SIN.match(string).to_s
  end
  result
end

# puts "grab_sin returns a SIN if the string has a SIN"
...
def hide_all_sins(string)
  sin_numbers = []
  if has_sin?(string)
    sin_numbers = string.gsub(/(\d{3})-(\d{3})/,"XXX-XXX")
  end 
  sin_numbers
...
def format_sins(string)
  sin_numbers = string.split(",") # turns it to an array
  
  sin_array = sin_numbers.map do |sin| # goes through each set of sin and replaces any delimeters with a blank
    sin.gsub(/\D/,"")
  end

  sin_array.each do |sin|
    if sin =~ REGEX
      puts sin.gsub(/(\d{3})\.?(\d{3})\.?(\d{3})/, "\\1-\\2-\\3") # finds every 3rd element and adds -
    end
  end
end
